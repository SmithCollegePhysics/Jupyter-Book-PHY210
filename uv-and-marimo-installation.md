...
1. ```uv add marimo``` installs the minimum necessary (so as to maximize compatibility with your own environments)
2. ```uv add marimo[recommended]``` adds additional features in the marimo editor, including SQL cells, AI completion, and server-side plotting of dataframe columns.
```
